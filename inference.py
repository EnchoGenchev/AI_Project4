...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        #get all factors with bayesNet
        factors = bayesNet.getAllCPTsWithEvidence(evidenceDict)

        #eliminate variables accoring to eliminationORder
        for var in eliminationOrder:
            #eliminate var by joining
            factors, joinedFactor = joinFactorsByVariable(factors, var)
            
            #handle ase where a factor you have joined only has one unconditioned variable
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        """
        "*** YOUR CODE HERE ***"        
        #get random sample from the distribution
        target = random.random() * self.total()
        sum_ = 0.0 #_ bc sum is funct in python
        
        #add up values until pass the target
        for key, value in self.items():
            sum_ += value
            if sum_ >= target: 
                return key #returning key that was sampled from
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        jail = self.getJailPosition()
        # build weighted distribution based on how likely each particle is given the observation
        weightedDist = DiscreteDistribution()
        for particle in self.particles:
            # weight each particle by the probability of seeing this observation from its position
            obsProb = self.getObservationProb(observation, pacPosition, particle, jail)
            weightedDist[particle] += obsProb
        # if no particles got any weight, start over with a uniform distribution
        if sum(weightedDist.values()) == 0:
            self.initializeUniformly(gameState)
        else:
            # normalize so weights become probabilities
            weightedDist.normalize()
            # resample particles from the weighted distribution
            self.particles = [weightedDist.sample() for _ in range(self.numParticles)]
        "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            #obtains the distribution over new positions for the ghost
            newPosDist = self.getPositionDistribution(gameState, oldPos)
        ...
        #assign back new list to self.particles
